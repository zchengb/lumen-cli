...
class VisualDeliveryTests(unittest.TestCase):
    ...
    def test_set_visual_auth_writes_secret_to_repo_runtime_and_clears_legacy_env(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            lumen = root / "lumen"
            (lumen / "config").mkdir(parents=True)
            ...
            self.assertEqual("STORED-WIW", payload["repositories"][0]["runtime"]["visual_auth_credential"])
            self.assertNotIn("auth_credential_env", payload["repositories"][0]["runtime"])
            env_path = lumen / ".env.local"
            if env_path.exists():
                self.assertNotIn("LUMEN_VISUAL_AUTH_DIGITAL_PLATFORM_ADMIN=", env_path.read_text())
