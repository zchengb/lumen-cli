...
def extract_project_slug(text: str, known_slugs: Optional[set[str]] = None) -> Optional[str]:
    ...
    for match in _SLUG_RE.finditer(text or ""):
        ...
        if candidate in {"scan", "status", "cancel", "dylan", "lumen", "high", "days"}:
            continue
...
def parse_dylan_text(text: str, known_slugs: Optional[set[str]] = None) -> ParsedAction:
    ...
    if _CANCEL_RE.search(cleaned):
        return ParsedAction(name="scan.cancel", confidence=0.9, source="rule:cancel")
    if _STATUS_RE.search(cleaned):
        return ParsedAction(name="scan.status", confidence=0.85, source="rule:status")
    ...
    if _SCAN_RE.search(cleaned) or params:
        return ParsedAction(
            name="scan.run",
            confidence=0.8 if _SCAN_RE.search(cleaned) else 0.6,
            source="rule:scan",
            params=params,
        )
