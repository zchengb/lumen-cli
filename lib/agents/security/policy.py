...
from agents.security.actions import DYLAN_ACTIONS, MARK_ACTIONS
...
def load_access_config(config: Optional[dict[str, Any]] = None) -> dict[str, Any]:
    ...
    return {
        "allowed_chat_ids": [str(x).strip() for x in (access.get("allowed_chat_ids") or []) if str(x).strip()],
        "allowed_user_ids": [str(x).strip() for x in (access.get("allowed_user_ids") or []) if str(x).strip()],
        "mutation_allowed_user_ids": [
            str(x).strip() for x in (access.get("mutation_allowed_user_ids") or []) if str(x).strip()
        ],
        "admin_user_ids": [str(x).strip() for x in (access.get("admin_user_ids") or []) if str(x).strip()],
    }
