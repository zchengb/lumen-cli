#!/usr/bin/env python3
"""Manage macOS launchd timers for Auto Patch polling."""
...
def install(args: argparse.Namespace) -> int:
    ...
    if interval is None:
        print("Error: Auto Patch schedule must look like '*/N * * * *'.", file=sys.stderr)
        return 1
    ...
    payload = {
        "Label": label,
        "ProgramArguments": [args.lumen_bin, "patch", "run", "--project", args.project],
        "EnvironmentVariables": {"PATH": args.path, "HOME": str(Path.home()), "LUMEN_HOME": args.lumen_home, "AGENT_CLI_CREDENTIAL_STORE": "file"},
