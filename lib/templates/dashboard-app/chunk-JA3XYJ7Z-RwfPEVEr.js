...
import { I as r, N as i, O as a, s as o, y as s } from "./chunk-ABZYJK2D-CVevQ3ur.js";
import { i as c } from "./chunk-S3R3BYOJ-BtVK2Rr4.js";
...
function ie(e, t, n) {
	...
	return o(t), n.forEach(o), ne(e, a);
}
function ae(e, t) {
	if (e.icons[t]) return ie(e, t, []);
	let n = re(e, [t])[t];
	return n ? ie(e, t, n) : null;
}
//#endregion
...
var fe = (e) => e === "unset" || e === "undefined" || e === "none";
function pe(e, t) {
	let n = {
		...d,
		...e
	...
	o === null ? (f = s === null ? "1em" : s === "auto" ? l : s, u = ce(f, c / l)) : (u = o === "auto" ? c : o, f = s === null ? ce(u, l / c) : s === "auto" ? l : s);
	let p = {}, m = (e, t) => {
		fe(t) || (p[e] = t.toString());
	};
...
function ge(e) {
	e = e.replace(/[0-9]+$/, "") || "a";
	let t = he.get(e) || 0;
	return he.set(e, t + 1), t ? `${e}${t}` : e;
}
function _e(e) {
	let t = [], n;
	for (; n = me.exec(e);) t.push(n[1]);
	if (!t.length) return e;
	let r = "suffix" + (Math.random() * 16777216 | Date.now()).toString(16);
	return t.forEach((t) => {
		let n = ge(t), i = t.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
...
var be = (() => {
	...
	table: v,
	text: Oe
}, Me = y("^ *([^\\n ].*)\\n {0,3}((?:\\| *)?:?-+:? *(?:\\| *:?-+:? *)*(?:\\| *)?)(?:\\n((?:(?! *\\n|hr|heading|blockquote|code|fences|list|html).*(?:\\n|$))*)\\n*|$)").replace("hr", x).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("blockquote", " {0,3}>").replace("code", "(?: {4}| {0,3}	)[^\\n]").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", T).getRegex(), Ne = {
	...O,
	lheading: De,
	table: Me,
	paragraph: y(C).replace("hr", x).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("table", Me).replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", T).getRegex()
}, Pe = {
	...O,
	...
	...M,
	link: y(/^!?\[(label)\]\((.*?)\)/).replace("label", j).getRegex(),
	reflink: y(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label", j).getRegex()
}, N = {
	...M,
	emStrongRDelimAst: Xe,
	emStrongLDelim: qe,
	url: y(/^((?:protocol):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/).replace("protocol", ot).replace("email", /[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![-_])/).getRegex(),
	_backpedal: /(?:[^?!.,:;*_'"~()&]+|\([^)]*\)|&(?![a-zA-Z0-9]+;$)|[?!.,:;*_'"~)]+(?!$))+/,
	del: /^(~~?)(?=[^\s~])((?:\\[\s\S]|[^\\])*?(?:\\[\s\S]|[^\s~\\]))\1(?=[^~]|$)/,
	text: y(/^([`~]+|[^`~])(?:(?= {2,}\n)|(?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)|[\s\S]*?(?:(?=[\\<!\[`*~_]|\b_|protocol:\/\/|www\.|$)|[^ ](?= {2,}\n)|[^a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-](?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)))/).replace("protocol", ot).getRegex()
}, ct = {
	...N,
	br: y(Le).replace("{2,}", "*").getRegex(),
	text: y(N.text).replace("\\b_", "\\b_| {2,}\\n").replace(/\{2,\}/g, "*").getRegex()
}, P = {
	normal: O,
	gfm: Ne,
	pedantic: Pe
...
function I(e, t) {
	if (t) {
		if (b.escapeTest.test(e)) return e.replace(b.escapeReplace, ut);
	} else if (b.escapeTestNoEncode.test(e)) return e.replace(b.escapeReplaceNoEncode, ut);
	return e;
}
function dt(e) {
	try {
...
${c}` : c;
	...
	inlineQueue;
	constructor(e) {
		this.tokens = [], this.tokens.links = Object.create(null), this.options = e || _, this.options.tokenizer = this.options.tokenizer || new R(), this.tokenizer = this.options.tokenizer, this.tokenizer.options = this.options, this.tokenizer.lexer = this, this.inlineQueue = [], this.state = {
			inLink: !1,
			inRawBlock: !1,
			top: !0
		};
...
`;
	...
	textRenderer;
	constructor(e) {
		this.options = e || _, this.options.renderer = this.options.renderer || new B(), this.renderer = this.options.renderer, this.renderer.options = this.options, this.renderer.parser = this, this.textRenderer = new V();
	}
	static parse(t, n) {
...
G.options = G.setOptions = function(e) {
	return W.setOptions(e), G.defaults = W.defaults, ye(G.defaults), G;
}, G.getDefaults = g, G.defaults = _, G.use = function(...e) {
	return W.use(...e), G.defaults = W.defaults, ye(G.defaults), G;
...
var _t = {
	...
	let o = pe(a, n);
	return r(ve(_e(o.body), {
		...o.attributes,
		...i
	}), s());
}, "getIconSVG");
function Ct(e, { markdownAutoWrap: t }) {
	let n = gt(e.replace(/<br\/>/g, "\n").replace(/\n{2,}/g, "\n"));
...
function Tt(n, { markdownAutoWrap: r } = {}) {
	let i = G.lexer(n);
	function a(t) {
...
t(q, "splitTextToChars");
function Et(e, t) {
	return J(e, [], q(t.content), t.type);
}
t(Et, "splitWordToFitWidth");
function J(e, t, n, r) {
	...
	return e([{
		content: o.join(""),
		type: r
	}]) ? J(e, o, a, r) : (t.length === 0 && i && (t.push(i), n.shift()), [{
		content: t.join(""),
		type: r
	}, {
		content: n.join(""),
...
t(X, "applyStyle");
async function Ot(e, t, n, c, l = !1, u = s()) {
	let d = e.append("foreignObject");
	d.attr("width", `${10 * n}px`), d.attr("height", `${10 * n}px`);
	let f = d.append("xhtml:div"), p = a(t.label) ? await i(t.label.replace(o.lineBreakRegex, "\n"), u) : r(t.label, u), ee = t.isNode ? "nodeLabel" : "edgeLabel", m = f.append("span");
	m.html(p), X(m, t.labelStyle), m.attr("class", `${ee} ${c}`), X(f, t.labelStyle), f.style("display", "table-cell"), f.style("white-space", "nowrap"), f.style("line-height", "1.5"), f.style("max-width", n + "px"), f.style("text-align", "center"), f.attr("xmlns", "http://www.w3.org/1999/xhtml"), l && f.attr("class", "labelBkg");
	let h = f.node().getBoundingClientRect();
	return h.width === n && (f.style("display", "table"), f.style("white-space", "break-spaces"), f.style("width", n + "px"), h = f.node().getBoundingClientRect()), d.node();
}
t(Ot, "addHtmlSpan");
function Z(e, t, n) {
...
function jt(e, n, r, i = !1) {
	let a = 1.1, o = n.append("g"), s = o.insert("rect").attr("class", "background").attr("style", "stroke: none"), c = o.append("text").attr("y", "-10.1"), l = 0;
	for (let n of r) {
		let r = /* @__PURE__ */ t((t) => kt(o, a, t) <= e, "checkWidth"), i = r(n) ? [n] : Dt(n, r);
		for (let e of i) Q(Z(c, l, a), e), l++;
	}
	if (i) {
		let e = c.node().getBBox();
		return s.attr("x", e.x - 2).attr("y", e.y - 2).attr("width", e.width + 4).attr("height", e.height + 4), o.node();
	} else return c.node();
}
...
var Mt = /* @__PURE__ */ t(async (t, r = "", { style: i = "", isTitle: o = !1, classes: s = "", useHtmlLabels: l = !0, isNode: u = !0, width: d = 200, addSvgBackground: f = !1 } = {}, p) => {
	if (e.debug("XYZ createText", r, i, o, s, l, u, "addSvgBackground: ", f), l) {
		let e = await $(c(Tt(r, p)), p), n = r.replace(/\\\\/g, "\\");
		return await Ot(t, {
			isNode: u,
			label: a(r) ? n : e,
		...
		let e = jt(d, t, wt(r.replace(/<br\s*\/?>/g, "<br/>").replace("<br>", "<br/>"), p), r ? f : !1);
		if (u) {
			...
			let t = i.replace(/stroke:[^;]+;?/g, "").replace(/stroke-width:[^;]+;?/g, "").replace(/fill:[^;]+;?/g, "").replace(/background:/g, "fill:");
			n(e).select("rect").attr("style", t.replace(/background:/g, "fill:"));
			let r = i.replace(/stroke:[^;]+;?/g, "").replace(/stroke-width:[^;]+;?/g, "").replace(/fill:[^;]+;?/g, "").replace(/color:/g, "fill:");
			n(e).select("text").attr("style", r);
		}
		return e;
	}
}, "createText");
//#endregion
export { $ as a, yt as i, Mt as n, _t as o, St as r, gt as s, At as t };
