...
function VisualRuntimeEditor({ runtime, onChange }: { runtime: RecordValue; onChange: (patch: RecordValue) => void }) {
  ...
  return <section className="repository-runtime">
    <div className="repository-runtime-heading"><div><strong>Visual runtime <HelpTip>These settings tell Lumen how to start the repository, reach its page, authenticate, and provide mock data during visual verification.</HelpTip></strong><span>Only the runtime inputs needed for visual verification are shown first.</span></div><code>{runtime.runtime_profile || platform}</code></div>
    <div className="form-grid compact">
      <Field label="Base URL" help="The address the visual agent opens, for example http://127.0.0.1:3000."><input value={runtime.base_url || ""} placeholder="http://127.0.0.1:3000" onChange={(event) => onChange({ base_url: event.target.value })} /></Field>
      <Field label="Ready URL" help="The address Lumen polls before handing the page to the visual agent. Keep it on the same running app unless a separate health route is needed."><input value={runtime.ready_url || ""} placeholder="http://127.0.0.1:3000" onChange={(event) => onChange({ ready_url: event.target.value })} /></Field>
      <Field label="Auth strategy" help="Choose how the agent becomes authenticated: login endpoint submits the configured credential, storage state reuses a saved session, and existing session connects to an already logged-in browser."><select value={runtime.auth_strategy || "login-endpoint"} onChange={(event) => onChange({ auth_strategy: event.target.value })}><option value="login-endpoint">Login endpoint</option><option value="storage-state">Storage state</option><option value="existing-session">Existing session</option></select></Field>
      <Field label="Visual auth credential" help="The local repository credential used by the visual login flow. It is shown here in plain text for local debugging and is not included in agent prompts or evidence."><input type="text" autoComplete="off" value={runtime.visual_auth_credential || ""} placeholder="Enter repository login credential" onChange={(event) => onChange({ visual_auth_credential: event.target.value })} /></Field>
      <Field label="Auth login path" help="The relative API path used by the login endpoint, such as /oauth-proxy-api/auth/admin/fake."><input value={runtime.auth_login_path || ""} placeholder="/auth/fake" onChange={(event) => onChange({ auth_login_path: event.target.value })} /></Field>
      <Field label="Auth login field" help="The request field expected by the login endpoint, such as wiw or username."><input value={runtime.auth_login_field || ""} placeholder="username" onChange={(event) => onChange({ auth_login_field: event.target.value })} /></Field>
      <Field label="Fixture strategy" help="Controls how API responses are made deterministic during visual verification. Playwright route is the usual choice for a browser app."><select value={runtime.fixture_strategy || "playwright-route"} onChange={(event) => onChange({ fixture_strategy: event.target.value })}><option value="playwright-route">Playwright route</option><option value="mock-server">Mock server</option><option value="none">None</option></select></Field>
      <Field label="Fixture file" help="The mock-data file loaded by the selected fixture strategy. Use a repository-relative path."><input value={runtime.fixture_file || ""} placeholder="stories/.../visual-fixtures.json" onChange={(event) => onChange({ fixture_file: event.target.value })} /></Field>
    </div>
    <details className="repository-runtime-advanced"><summary>Advanced runtime settings</summary>
      <div className="form-grid compact">
        <Field label="Platform"><select value={platform} onChange={(event) => onChange({ platform: event.target.value })}><option value="web">Web</option><option value="react-native">React Native</option></select></Field>
        <Field label="Package manager"><input value={runtime.package_manager || ""} placeholder="yarn" onChange={(event) => onChange({ package_manager: event.target.value })} /></Field>
        <Field label="Install command"><input value={runtime.install_command || ""} placeholder="yarn install" onChange={(event) => onChange({ install_command: event.target.value })} /></Field>
        <Field label="Start command"><input value={runtime.start_command || ""} placeholder="PORT=3000 yarn start:dev" onChange={(event) => onChange({ start_command: event.target.value })} /></Field>
        <Field label="Ready timeout, seconds"><input type="number" min="1" value={runtime.ready_timeout_seconds || 60} onChange={(event) => onChange({ ready_timeout_seconds: Number(event.target.value) || 60 })} /></Field>
        {platform === "web" && <><Field label="Browser"><input value={runtime.browser || "chromium"} onChange={(event) => onChange({ browser: event.target.value })} /></Field><Field label="Browser mode"><select value={runtime.browser_mode || "managed"} onChange={(event) => onChange({ browser_mode: event.target.value })}><option value="managed">Managed</option><option value="cdp">Chrome CDP</option></select></Field></>}
      </div>
      {platform === "web" && <Field label="Agent auth notes"><textarea value={runtime.agent_auth_notes || ""} rows={2} placeholder="Same-origin, cookie, or local runtime notes" onChange={(event) => onChange({ agent_auth_notes: event.target.value })} /></Field>}
    </details>
  </section>;
}
    ...
    <Panel title="Repository Registry" action={<button className="button secondary" onClick={() => setAddOpen(true)}>Add repository</button>}>
      ...
      <div className="repository-list">{repositories.map((repository, index) => {
        ...
        return <article className="repository-row" key={`${repository.name}-${index}`}>
          ...
          {open && <div className="repository-editor"><div className="form-grid compact"><Field label="Default branch"><select value={repository.default_branch || ""} onChange={(event) => update(index, { default_branch: event.target.value })}>{Array.from(new Set([repository.default_branch, ...(repository.branches || [])].filter(Boolean))).map((branch) => <option value={branch} key={branch}>{branch}</option>)}</select></Field><Field label="Scan runtime profile"><select value={repository.runtime_profile || ""} onChange={(event) => update(index, { runtime_profile: event.target.value })}>{profileNames.map((name) => <option value={name} key={name}>{name}</option>)}</select></Field><Field label="Visual runtime status" help="Ready tells Lumen this repository has enough startup, authentication, and fixture configuration for visual verification."><select value={repository.runtime_status || "incomplete"} onChange={(event) => update(index, { runtime_status: event.target.value })}><option value="ready">Ready</option><option value="incomplete">Incomplete</option></select></Field></div><div className="repository-policy"><label><input type="checkbox" checked={repository.allow_auto_fix !== false} onChange={(event) => update(index, { allow_auto_fix: event.target.checked })} />Allow automated Scan fixes</label><label><input type="checkbox" checked={repository.allow_pr !== false} onChange={(event) => update(index, { allow_pr: event.target.checked })} />Allow publish</label><span>Local Scan commands: disabled (review-only)</span></div><VisualRuntimeEditor runtime={repository.runtime || {}} onChange={(patch) => update(index, { runtime: { ...(repository.runtime || {}), ...patch } })} /><details className="repository-verification-advanced"><summary>Delivery verification commands</summary><label className="field repository-commands"><textarea value={repository.delivery_commands ?? commandsFor(repository)} rows={4} placeholder="One command per line, e.g. ./gradlew test" onChange={(event) => update(index, { delivery_commands: event.target.value })} /></label></details></div>}
        </article>;
      })}</div>
