...
function DeliveryFlow({ stages, deliveryStatus, startedAt, finishedAt, onStageClick }: { stages: RecordValue[]; deliveryStatus: string; startedAt?: string; finishedAt?: string; onStageClick: (stage: RecordValue) => void }) {
  ...
  return <div className="delivery-flow"><div className="flow-heading"><div><strong>Delivery Flow</strong></div><p>{startedAt ? `Started ${when(startedAt)}` : "Awaiting delivery trigger"}{finishedAt ? ` · Finished ${when(finishedAt)}` : ""}</p></div><div className="flow-track-wrap"><span className="flow-track"><i style={{ width: `${progress}%` }} /></span><ol className="flow-steps" style={{ "--flow-count": stages.length } as React.CSSProperties}>{stages.map((stage, index) => {
