...
function SettingsView({ data, project, notify, onDirtyChange, reload }: { data: DashboardData; project: string; notify: (message: string) => void; onDirtyChange: (dirty: boolean) => void; reload: () => Promise<void> }) {
  ...
  return <div className="settings-stack">
    ...
    <footer className="settings-save-bar"><span>{dirty ? "You have unsaved changes" : "All changes saved"}</span><button className="button primary" disabled={!dirty} onClick={() => void saveAll()}><Save size={15} />Save changes</button></footer>
  </div>;
}
