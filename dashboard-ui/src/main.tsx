...
function VisualRuntimeEditor({ runtime, credentialConfigured, onChange }: { runtime: RecordValue; credentialConfigured: boolean; onChange: (patch: RecordValue) => void }) {
  const platform = String(runtime.platform || "web");
  return <section className="repository-runtime">
    <div className="repository-runtime-heading"><div><strong>Visual runtime</strong><span>Shared browser/device startup, readiness, auth, and mock-data settings for this repository.</span></div><code>{runtime.runtime_profile || platform}</code></div>
    <div className="form-grid compact">
      ...
      <Field label="Base URL"><input value={runtime.base_url || ""} placeholder="http://127.0.0.1:3000" onChange={(event) => onChange({ base_url: event.target.value })} /></Field>
      <Field label="Ready URL"><input value={runtime.ready_url || ""} placeholder="http://127.0.0.1:3000" onChange={(event) => onChange({ ready_url: event.target.value })} /></Field>
    ...
    {platform === "web" && <>
      <div className="form-grid compact">
        <Field label="Browser"><input value={runtime.browser || "chromium"} onChange={(event) => onChange({ browser: event.target.value })} /></Field>
        <Field label="Browser mode"><select value={runtime.browser_mode || "managed"} onChange={(event) => onChange({ browser_mode: event.target.value })}><option value="managed">Managed</option><option value="cdp">Chrome CDP</option></select></Field>
        <Field label="Auth strategy"><select value={runtime.auth_strategy || "login-endpoint"} onChange={(event) => onChange({ auth_strategy: event.target.value })}><option value="login-endpoint">Login endpoint</option><option value="storage-state">Storage state</option><option value="existing-session">Existing session</option></select></Field>
        <Field label="Visual auth credential"><input type="password" autoComplete="new-password" value={runtime.visual_auth_credential || ""} placeholder={credentialConfigured ? "Configured; enter a replacement" : "Enter repository login credential"} onChange={(event) => onChange({ visual_auth_credential: event.target.value })} /></Field>
        <Field label="Auth login path"><input value={runtime.auth_login_path || ""} placeholder="/auth/fake" onChange={(event) => onChange({ auth_login_path: event.target.value })} /></Field>
        <Field label="Auth login field"><input value={runtime.auth_login_field || ""} placeholder="username" onChange={(event) => onChange({ auth_login_field: event.target.value })} /></Field>
        <Field label="Fixture strategy"><select value={runtime.fixture_strategy || "playwright-route"} onChange={(event) => onChange({ fixture_strategy: event.target.value })}><option value="playwright-route">Playwright route</option><option value="mock-server">Mock server</option><option value="none">None</option></select></Field>
        <Field label="Fixture file"><input value={runtime.fixture_file || ""} placeholder="stories/.../visual-fixtures.json" onChange={(event) => onChange({ fixture_file: event.target.value })} /></Field>
      </div>
      <Field label="Agent auth notes"><textarea value={runtime.agent_auth_notes || ""} rows={2} placeholder="Same-origin, cookie, or local runtime notes" onChange={(event) => onChange({ agent_auth_notes: event.target.value })} /></Field>
    </>}
  </section>;
}
