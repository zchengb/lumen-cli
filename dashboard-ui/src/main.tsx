...
function DeliveryView({ data, project }: { data: DashboardData; project: string }) {
  ...
  const loadDeliveryLog = useCallback(async (runId = current.run_id || "") => {
    setLoadingLog(true);
  ...
  useEffect(() => { if (!selectedStage || !running) return; void loadDeliveryLog(); const id = window.setInterval(() => void loadDeliveryLog(), 2_000); return () => window.clearInterval(id); }, [selectedStage, running, loadDeliveryLog]);
  const openStage = async (stage: RecordValue, runId = current.run_id || "") => {
    setSelectedStage(stage); setLogContent(""); setLogError(""); await loadDeliveryLog(runId);
  };
...
function DeliveryFlow({ stages, deliveryStatus, startedAt, finishedAt, onStageClick }: { stages: RecordValue[]; deliveryStatus: string; startedAt?: string; finishedAt?: string; onStageClick: (stage: RecordValue) => void }) {
  ...
  return <div className="delivery-flow"><div className="flow-heading"><div><span className="flow-title">Delivery Flow</span></div><p>{startedAt ? `Started ${when(startedAt)}` : "Awaiting delivery trigger"}{finishedAt ? ` · Finished ${when(finishedAt)}` : ""}</p></div><div className="flow-track-wrap"><span className="flow-track"><i style={{ width: `${progress}%` }} /></span><ol className="flow-steps" style={{ "--flow-count": stages.length } as React.CSSProperties}>{stages.map((stage, index) => {
...
function SettingsView({ data, project, interact, notify }: { data: DashboardData; project: string; interact: (path: string, json: RecordValue, message: string) => Promise<void>; notify: (message: string) => void }) {
  ...
  return <div className="settings-stack">
    <Panel title="Workspace" action={<button className="button primary" onClick={() => void interact("/api/workspace", { scan_window_days: Number(scanWindow) }, "Workspace configuration saved")}><Save size={15} />Save</button>}><div className="settings-section"><div className="settings-copy"><h4>Review window</h4><p>Controls how much repository history Auto Scan considers on each run.</p></div><div className="settings-control"><Field label="Lookback, days"><input type="number" min="1" max="365" value={scanWindow} onChange={(event) => setScanWindow(event.target.value)} /></Field></div></div><div className="settings-meta"><InfoRow label="Workspace" value={<code>{text(workspace.path)}</code>} /></div></Panel>
    <Panel title="Automation schedules">
      <div className="settings-section"><div className="settings-copy"><div className="settings-heading"><div className="settings-title-stack"><h4>Auto Scan</h4><ScheduleToggle enabled={Boolean(schedules.scan)} onChange={(enabled) => enabled ? void interact("/api/schedule", { kind: "scan", action: "save", cron: scanCron }, "Auto Scan schedule enabled") : void interact("/api/schedule", { kind: "scan", action: "remove" }, "Auto Scan schedule paused")} /></div><button className="button primary" onClick={() => void interact("/api/schedule", { kind: "scan", action: "save", cron: scanCron }, "Auto Scan schedule saved")}><Save size={15} />Save</button></div><p>{text(schedules.scan?.description, "No recurring scan is configured.")}</p></div><div className="settings-control wide"><div className="cron-field"><Field label="Five-field cron"><input value={scanCron} onChange={(event) => setScanCron(event.target.value)} /></Field></div></div></div>
      <div className="settings-section divider"><div className="settings-copy"><div className="settings-heading"><div className="settings-title-stack"><h4>Auto Delivery</h4><ScheduleToggle enabled={Boolean(schedules.delivery?.enabled)} onChange={(enabled) => enabled ? saveDelivery() : void interact("/api/schedule", { kind: "delivery", action: "remove" }, "Auto Delivery schedule paused")} /></div><button className="button primary" onClick={saveDelivery}><Save size={15} />Save</button></div><p>{schedules.delivery?.enabled ? `Polling every ${Math.round(Number(schedules.delivery?.interval_seconds || 300) / 60)} minute(s).` : "Delivery polling is paused."}</p></div><div className="settings-control wide"><div className="form-grid compact"><Field label="Interval, minutes"><input type="number" min="1" value={deliveryInterval} onChange={(event) => setDeliveryInterval(event.target.value)} /></Field><Field label="Eligible JIRA status"><select value={deliveryStatus} onChange={(event) => setDeliveryStatus(event.target.value)}>{statusOptions.map((value) => <option value={value} key={value}>{value}</option>)}</select></Field><Field label="Move to when started"><select value={inDevStatus} onChange={(event) => setInDevStatus(event.target.value)}>{statusOptions.map((value) => <option value={value} key={value}>{value}</option>)}</select></Field><Field label="Move to when completed"><select value={devDoneStatus} onChange={(event) => setDevDoneStatus(event.target.value)}>{statusOptions.map((value) => <option value={value} key={value}>{value}</option>)}</select></Field></div><p className="schedule-note">On failure, Lumen leaves the JIRA status unchanged and adds a Needs attention comment.</p></div></div>
    </Panel>
