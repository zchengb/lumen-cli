...
function RepositoryView({ data, interact }: { data: DashboardData; interact: (path: string, json: RecordValue, message: string) => Promise<void> }) {
  ...
  const automationFor = (repository: RecordValue) => ({ scan: { allow_auto_fix: repository.automation?.scan?.allow_auto_fix ?? repository.allow_auto_fix !== false, allow_pr: repository.automation?.scan?.allow_pr ?? repository.allow_pr !== false }, delivery: { enabled: repository.automation?.delivery?.enabled !== false }, patch: { enabled: repository.automation?.patch?.enabled === true } });
  const updateAutomation = (index: number, section: "scan" | "delivery" | "patch", patch: RecordValue) => setRepositories((items) => items.map((item, current) => current === index ? { ...item, automation: { ...automationFor(item), [section]: { ...automationFor(item)[section], ...patch } } } : item));
  ...
  return <div className="repository-page">
    <Panel title="Repository Governance" action={<button className="button secondary" onClick={() => setAddOpen(true)}>Add repository</button>}>
      ...
      <div className="repository-overview"><Fact label="Registered" value={repositories.length} /><Fact label="Needs attention" value={attention} /><Fact label="Auto Patch authorized" value={patchEnabled} /><Fact label="Frontend delivery" value="Disabled globally" /></div>
      <div className="repository-filters"><button className={filter === "all" ? "active" : ""} onClick={() => setFilter("all")}>All ({repositories.length})</button><button className={filter === "attention" ? "active" : ""} onClick={() => setFilter("attention")}>Needs attention ({attention})</button><button className={filter === "patch" ? "active" : ""} onClick={() => setFilter("patch")}>Patch authorized ({patchEnabled})</button></div>
      <div className="repository-list">{visible.map((repository) => {
        ...
        return <article className="repository-row" key={`${repository.name}-${index}`}>
          <button className="repository-summary" onClick={() => setExpanded(open ? null : repository.name)}><div><strong>{repository.name || "Unnamed repository"}</strong><span>{health.language || "Generic"} · {health.build_tools?.join(", ") || "No build tool detected"}</span></div><span className="repository-summary-state"><Badge value={health.git_status || "unknown"} /><code>{repository.default_branch || "main"}</code></span></button>
          {open && <div className="repository-editor">
            <section className="repository-section"><div><strong>Identity & connection</strong><span>Detected locally; the default branch is the only editable connection setting.</span></div><div className="repository-facts"><Fact label="Local path" value={<code>{repository.path}</code>} /><Fact label="Remote" value={<code>{health.remote_url || repository.remote_url || "Not configured"}</code>} /><Fact label="Git status" value={<Badge value={health.git_status || "unknown"} />} /><Fact label="Branch sync" value={<Badge value={health.sync_status || "unknown"} />} /></div><div className="form-grid compact"><Field label="Default branch"><select value={repository.default_branch || ""} onChange={(event) => update(index, { default_branch: event.target.value })}>{Array.from(new Set([repository.default_branch, ...(repository.branches || [])].filter(Boolean))).map((branch) => <option value={branch} key={branch}>{branch}</option>)}</select></Field></div></section>
            <section className="repository-section"><div><strong>Runtime & build</strong><span>Detected from repository files. These values are read-only until the repository changes.</span></div><div className="repository-facts"><Fact label="Language" value={health.language || "Generic"} /><Fact label="Java" value={health.java_version ? `Java ${health.java_version}` : "Not detected"} /><Fact label="Node" value={health.node_version ? `Node ${health.node_version}` : "Not detected"} /><Fact label="Build tools" value={health.build_tools?.join(", ") || "Not detected"} /></div></section>
            <section className="repository-section"><div><strong>Automation permissions</strong><span>Each permission is enforced by its own runtime. Auto Patch is disabled by default.</span></div><div className="repository-policy-grid"><label><input type="checkbox" checked={automation.scan.allow_auto_fix} onChange={(event) => updateAutomation(index, "scan", { allow_auto_fix: event.target.checked })} /><span><strong>Auto Scan fixes</strong><small>Allow high-confidence Scan fixes in this repository.</small></span></label><label><input type="checkbox" checked={automation.scan.allow_pr} onChange={(event) => updateAutomation(index, "scan", { allow_pr: event.target.checked })} /><span><strong>Auto Scan pull requests</strong><small>Allow Scan fixes to be published as pull requests.</small></span></label><label><input type="checkbox" checked={automation.delivery.enabled} onChange={(event) => updateAutomation(index, "delivery", { enabled: event.target.checked })} /><span><strong>Auto Delivery</strong><small>Allow approved technical delivery work for this repository.</small></span></label><label><input type="checkbox" checked={automation.patch.enabled} onChange={(event) => updateAutomation(index, "patch", { enabled: event.target.checked })} /><span><strong>Auto Patch</strong><small>Allow Jira-driven fixes and publishing for this repository.</small></span></label></div><p className="repository-policy-note">Frontend delivery remains disabled globally and cannot be enabled here.</p></section>
            <section className="repository-section"><div><strong>Delivery verification</strong><span>Suggested commands are detected from the build. Saving confirms the commands that Auto Delivery will actually run.</span></div>{health.suggested_commands?.length > 0 && <button className="text-button" onClick={() => update(index, { delivery_commands: health.suggested_commands.join("\n") })}>Use {health.suggested_commands.length} suggested command{health.suggested_commands.length === 1 ? "" : "s"}</button>}<label className="field repository-commands"><textarea value={repository.delivery_commands ?? commandsFor(repository)} rows={4} placeholder="No command confirmed. Add one command per line." onChange={(event) => update(index, { delivery_commands: event.target.value })} /></label></section>
          </div>}
        </article>;
      })}{visible.length === 0 && <Empty label="No repositories match this view." />}</div>
      <footer className="repository-footer"><button className="button primary" onClick={() => void interact("/api/repositories", { repositories }, "Repository governance saved")}><Save size={15} />Save governance</button></footer>
    </Panel>
...
function AddRepositoryDialog({ onClose, onAdd }: { onClose: () => void; onAdd: (url: string) => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><section className="modal repository-modal" role="dialog" aria-modal="true" aria-label="Add repository" onMouseDown={(event) => event.stopPropagation()}><div className="prompt-inspector-header"><div><strong>Add repository</strong><span>Lumen clones the Git URL, detects the branch and tooling, enables existing Scan and Delivery behavior, and leaves Auto Patch off for your approval.</span></div></div><div className="repository-modal-body"><Field label="Clone URL"><input autoFocus value={url} placeholder="https://git.example.com/team/service.git" onChange={(event) => setUrl(event.target.value)} /></Field></div><footer><button className="button" onClick={onClose}>Cancel</button><button className="button primary" disabled={!url.trim()} onClick={() => onAdd(url.trim())}>Clone and inspect</button></footer></section></div>;
}
