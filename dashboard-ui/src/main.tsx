  ...
  return <div className="repository-page">
      ...
      <div className="repository-list"><div className="repository-grid">{visible.map((repository) => {
        ...
        return <article className="repository-card" key={repository.name}>
          <button type="button" className="repository-card-button" onClick={() => setEditing(repository.name)} aria-label={`Edit ${repository.name || "repository"}`}>
            ...
            <div className="repository-card-meta"><Badge value={health.git_status || "unknown"} /><span><GitBranch size={12} aria-hidden="true" />{repository.default_branch || "main"}</span></div>
            <div className="repository-card-permissions"><span className={automation.patch.enabled ? "enabled" : "disabled"}>Auto Patch {automation.patch.enabled ? "enabled" : "disabled"}</span><span>{health.sync_status || "Sync status unknown"}</span></div>
          </button>
        </article>;
    ...
    {selectedRepository && selectedIndex >= 0 && selectedAutomation && <div className="modal-backdrop repository-config-backdrop" role="presentation" onMouseDown={() => setEditing(null)}><section className="modal repository-config-modal" role="dialog" aria-modal="true" aria-labelledby="repository-config-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="repository-config-body"><div className="repository-editor">
        ...
        <section className="repository-section"><div><strong>Delivery verification</strong><span>Leave this empty to use the repository profile detected at run time. Add commands only when this repository needs an explicit override.</span></div>{selectedHealth.suggested_commands?.length > 0 && <button className="text-button" onClick={() => update(selectedIndex, { delivery_commands: selectedHealth.suggested_commands.join("\n") })}>Use {selectedHealth.suggested_commands.length} suggested command{selectedHealth.suggested_commands.length === 1 ? "" : "s"}</button>}<label className="field repository-commands"><textarea value={selectedRepository.delivery_commands ?? commandsFor(selectedRepository)} rows={4} placeholder="Optional override; one command per line." onChange={(event) => update(selectedIndex, { delivery_commands: event.target.value })} /></label></section>
      </div></div>
    </section></div>}
