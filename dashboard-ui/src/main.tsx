...
function AgentTraceOverview({ trace, delivery, onOpen }: { trace: RecordValue; delivery: RecordValue; onOpen: () => void }) {
  ...
  const tools = invocations.reduce((sum: number, item: RecordValue) => sum + Number(item.tool_calls || 0), 0);
  const files = invocations.reduce((sum: number, item: RecordValue) => sum + Number(item.files_changed || 0), 0);
  const agentMs = invocations.reduce((sum: number, item: RecordValue) => sum + Number(item.duration_ms || 0), 0);
  const verificationSpans = spans.filter((item: RecordValue) => item.name === "code-verification");
  const remediationInvocations = invocations.filter((item: RecordValue) => item.stage === "remediation");
  const verificationMs = verificationSpans.length ? verificationSpans.reduce((sum: number, item: RecordValue) => sum + Number(item.duration_ms || 0), 0) : null;
  const remediationMs = remediationInvocations.length ? remediationInvocations.reduce((sum: number, item: RecordValue) => sum + Number(item.duration_ms || 0), 0) : null;
  return <Panel title="Agent Trace" className="agent-trace-panel" action={<button className="button secondary" onClick={onOpen}><Activity size={14} />Inspect local evidence</button>}><p className="trace-local-note"><ShieldCheck size={14} />Redacted local debugging evidence · {text(trace.capture_mode)} capture</p><div className="trace-metrics"><Fact label="Delivery duration" value={elapsed(delivery.started_at, delivery.finished_at)} /><Fact label="Agent duration" value={durationMs(agentMs)} /><Fact label="Verification" value={durationMs(verificationMs)} /><Fact label="Remediation" value={durationMs(remediationMs)} /><Fact label="Invocations" value={invocations.length} /><Fact label="Tool calls · files" value={`${tools} · ${files}`} /></div><div className="trace-invocation-strip">{invocations.map((item: RecordValue) => <span key={item.invocation_id}><code>{item.invocation_id}</code><Badge value={item.status} /><small>{durationMs(item.duration_ms)} · {titleStatus(item.trace_completeness)}</small></span>)}</div></Panel>;
}
...
function AgentTraceDialog({ payload, error, loading, onClose }: { payload: RecordValue; error: string; loading: boolean; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><section className="modal trace-modal" role="dialog" aria-modal="true" aria-label="Agent Trace" onMouseDown={(event) => event.stopPropagation()}><div className="delivery-log-header"><div><span>Agent Trace · local evidence</span><strong>{text(payload.trace?.trace_id, "Loading…")}</strong><p>{text(payload.trace?.capture_mode)} capture · redacted before persistence</p></div><button className="button secondary" onClick={onClose}>Close</button></div>{loading ? <div className="loading-state"><LoaderCircle size={20} className="spin" />Loading trace…</div> : error ? <div className="status-note"><CircleAlert size={15} />{error}</div> : <><div className="trace-toolbar"><select value={selected.invocation_id || ""} onChange={(event) => setSelectedId(event.target.value)} aria-label="Agent invocation">{invocations.map((item: RecordValue) => <option value={item.invocation_id} key={item.invocation_id}>{item.invocation_id}</option>)}</select><div className="trace-tabs" role="tablist">{["input", "events", "output", "changes"].map((value) => <button className={tab === value ? "active" : ""} onClick={() => setTab(value)} key={value}>{titleStatus(value)}</button>)}</div></div><div className="trace-detail">{tab === "input" && <><div className="trace-facts"><Fact label="Stage" value={text(requestData.stage)} /><Fact label="Attempt" value={text(requestData.attempt)} /><Fact label="Model" value={text(requestData.model)} /><Fact label="Prompt size" value={`${Number(requestData.prompt_characters || 0).toLocaleString()} chars`} /><Fact label="Prompt hash" value={<code>{text(requestData.prompt_sha256).slice(0, 12)}…</code>} /><Fact label="Workspace" value={<code>{text(requestData.workspace)}</code>} /></div><h4>Context sources</h4><div className="trace-sources">{(selected.context_manifest?.sources || []).map((source: RecordValue, index: number) => <span key={`${source.path}-${index}`}><Badge value={source.included ? "included" : "missing"} /><code>{text(source.type)}</code><small>{text(source.path)}</small></span>)}</div>{selected.prompt ? <details><summary>Redacted composed prompt{selected.truncated?.prompt ? " · truncated" : ""}</summary><pre><code>{selected.prompt}</code></pre></details> : <Empty label="Prompt body is available only in full capture mode." />}</>}{tab === "events" && <><input className="trace-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search events, tools, paths, or errors" /><div className="trace-timeline">{events.map((event: RecordValue) => <article key={event.event_id}><time>{when(event.timestamp)}</time><span className={`timeline-dot ${statusTone(event.status || event.type)}`} /><div><strong>{text(event.type)}</strong><p>{text(event.tool?.name || event.message || event.error, "")}</p><small>{event.duration_ms != null ? durationMs(event.duration_ms) : ""}</small></div></article>)}</div></>}{tab === "output" && <><div className="trace-facts"><Fact label="Status" value={<Badge value={result.status} />} /><Fact label="Exit code" value={text(result.exit_code)} /><Fact label="Session" value={<code>{text(result.session_id)}</code>} /><Fact label="Duration" value={durationMs(result.duration_ms)} /><Fact label="First output" value={durationMs(result.time_to_first_output_ms)} /><Fact label="API duration" value={durationMs(result.provider_api_duration_ms)} /></div>{[["Final response", selected.final_output, "final_output"], ["stdout", selected.stdout, "stdout"], ["stderr", selected.stderr, "stderr"]].map(([label, content, key]) => content ? <details key={String(key)}><summary>{String(label)}{selected.truncated?.[String(key)] ? " · truncated" : ""}</summary><pre><code>{String(content)}</code></pre></details> : null)}</>}{tab === "changes" && <div className="trace-files">{files.length ? files.map((file: RecordValue) => <article key={`${file.repository}-${file.path}`}><Badge value={file.status} /><code>{file.repository}/{file.path}</code><span>+{text(file.additions, "?")} −{text(file.deletions, "?")}</span></article>) : <Empty label="No files were attributed to this invocation." />}</div>}</div></>}</section></div>;
}
...
function SettingsView({ data, project, interact, notify }: { data: DashboardData; project: string; interact: (path: string, json: RecordValue, message: string) => Promise<void>; notify: (message: string) => void }) {
  ...
  const saveObservability = () => void interact("/api/observability", { capture_mode: traceMode, retention_days: Number(traceRetention) }, "Agent Trace settings saved");
  return <div className="settings-stack">
    ...
    <Panel title="Agent Trace"><div className="settings-section"><div className="settings-copy"><h4>Local Delivery evidence</h4><p>Metadata is the safe default. Full mode stores redacted prompts, provider events, stdout, and stderr for local debugging.</p></div><div className="settings-control wide"><div className="form-grid compact"><Field label="Capture mode"><select value={traceMode} onChange={(event) => setTraceMode(event.target.value)}><option value="off">Off</option><option value="metadata">Metadata</option><option value="full">Full</option></select></Field><Field label="Retention, days"><input type="number" min="1" max="3650" value={traceRetention} onChange={(event) => setTraceRetention(event.target.value)} /></Field></div></div><div className="settings-footer" style={{ paddingBottom: 7 }}><button className="button primary" onClick={saveObservability}><Save size={15} />Save</button></div></div></Panel>
