...
function PatchFlow({ phases }: { phases: RecordValue[] }) {
  return <div className="delivery-flow patch-flow"><div className="flow-heading"><div><span className="flow-title">Patch Flow</span></div><p>Capture → screen → context → patch → publish</p></div><div className="flow-track-wrap"><span className="flow-track"><i style={{ width: `${phases.length ? Math.round(phases.filter((phase) => phase.status === "completed").length / phases.length * 100) : 0}%` }} /></span><ol className="flow-steps" style={{ "--flow-count": Math.max(phases.length, 1) } as React.CSSProperties}>{phases.map((phase, index) => { const status = String(phase.status || "pending").toLowerCase(); const state = status === "completed" ? "completed" : /in_progress|running/.test(status) ? "running" : /failed|blocked/.test(status) ? "failed" : "pending"; return <li className={`flow-step ${state}`} key={phase.id || index}><div className="flow-stage-button"><span className="flow-marker">{state === "completed" ? "✓" : index + 1}</span><span className="flow-copy"><strong>{text(phase.label)}</strong><span>{text(phase.detail || phase.status, "Pending")}</span></span></div></li>; })}</ol></div></div>;
}
